...
def describe_automaton(transitions):
    ...
    for keys, values in transitions.items():
        values = transitions.get(keys)
        print(f"When in state \""+keys[0]+"\" and processing \""+str(keys[1])+"\", automaton\'s state becomes \""+values+"\".")
...
def transitions_as_dict(transitions_as_list):
    transitions = {}
    for tran in transitions_as_list:
       import re
       t=re.split(',|:',tran)
       transitions[t[0],int(t[1])]= t[2]   
    return (transitions)
...
def accepts(transitions, word, initial_state, accept_state):
    ...
    '''
def accepts(transitions, word, initial_state, accept_state):
    k=list(transitions.keys())
    v=list(transitions.values())
    for i in range(len(word)):
        a=int(word[i])
        if (initial_state,a) not in k:
            return False
        elif transitions[initial_state,a]!=accept_state and i == len(word)-1:
            return False
        elif transitions[initial_state,a]==accept_state and i == len(word)-1:
            return True                  
        else:
            initial_state=transitions[initial_state,a]
    return accepts
